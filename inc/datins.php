...
switch( $_POST['dato'] ) {
	case 1:$datos = avisos();
	break;
	//case 2:$datos = cumples();
	//break;
	case 3:$datos = busqueda_avanzada();
	break; 
}
echo $datos;
...
//FUNCIONES
/**
 * DEPRECATED - Cambiar a variables.php
...
$total = mysql_numrows($consulta);
	if ($total >= 1)
	{
		...
		{
			$cadena .="<tr><td><a href='javascript:muestra(".$resultado[1].")'>".$resultado[5]."</a></td></tr>";
		}
	}
else
	$hnocump++;
...
$total = mysql_numrows($consulta);
	if ($total >= 1)
	{
		while($resultado = mysql_fetch_array($consulta))
		{
			$cadena .="<tr><td>".cambiaf($resultado[4])."</td><td><a href='javascript:muestra(".$resultado[1].")'>".$resultado[5]."</a></td></tr>";
		}
	}
else
	$hnocump++;
...
$total = mysql_numrows($consulta);
	if ($total >= 1)
	{
		while($resultado = mysql_fetch_array($consulta))
		{
			$cadena .="<tr><td>".cambiaf2($resultado[4])."</td><td><a href='javascript:muestra(".$resultado[1].")'>".$resultado[5]."</a></td></tr>";
		}
	}
else
	$hnocump++;
