...
function detallesCategoria ($vars)
{
    ...
    $cadena .= "Categoria: <input type='text' name='Nombre' value='" .
     traduce( $resultado[1] ) . "' size='40'/>";
    $cadena .= "<br/>Descripcion: <textarea name='descripcion' cols='35'>" .
     traduce( $resultado[2] ) . "</textarea>";
...
function actualizaCategoria ($vars)
{
    ...
    switch ($vars[categoria]) {
        case 1:
            $tabla = utf8_decode( "categoría servicios" );
            break;
        case 2:
    ...
    if ($consulta = mysql_db_query( $dbname, $sql, 
    $con ))
        $cadena = "todo ok";
    else
...
function listadoTelefonos ()
{
    ...
    while ($resultado = mysql_fetch_array( $consulta )) {
        if ($i % $columnas == 0)
            $cadena .= "</tr><tr>";
        $cadena .= "<td class='impar'>" . traduce( $resultado[0] ) .
    ...
    foreach ($no_asignados as $no_asignado) {
        ...
        if (! in_array( $no_asignado, $centro ))
            ...
            $cadena .= "<div class='" . $clase . "'>" . $no_asignado .
             "&nbsp;&nbsp;&nbsp;&nbsp;" . descripcion_telefono( $no_asignado ) .
             "</div>";
    }
    $cadena .= "</div>";
    //Fin telefonos Domiciliados
        ...
        if (is_array( $libres ))
            foreach ($libres as $libre) {
                ...
                $cadena .= "<div class='" . $clase .
                 "'><img src='iconos/edittrash.png' alt='Borrar telefono' onclick='javascript:borrar_telefono_asignado(\"" .
                 $libre .
                 "\")'>&nbsp;<img src='iconos/kate.png' alt='Editar telefono' onclick='javascript:editar_telefono_asignado(\"" .
                 $libre . "\")'>&nbsp;" . $libre .
...
function rarita ()
{
    ...
    while ($resultado = mysql_fetch_array( $consulta )) {
        ...
        $cadena .= "<tr><td bgcolor='" . $color .
         ...
         traduce( $resultado[1] ) . "</td><td class='" . clase( $k ) . "'>" .
         traduce( $resultado[2] ) . "</td><td class='" . clase( $k ) . "'>" .
         traduce( $resultado[3] ) . "</td></tr>";
        $k ++;
    }
    $cadena .= "</table>";
    return $cadena;
...
function listadoPersonalizado ($vars)
{
    ...
    while ($resultado = @mysql_fetch_array( $consulta )) {
        ...
        $cadena .= "<div class='" . $clase . "'>" . $i . " " .
         traduce( $resultado[1] ) . "<span class='direccion_esp'> " .
         traduce( $resultado[Direccion] ) . "</span></div>";
         //$i++;
    }
