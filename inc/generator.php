...
function codigoNegocio( $idemp = null )
{
	...
	if ( $idemp != null ) {
    	...
    	like " . $idemp ." and servicio like 'Codigo Negocio'";
	    $consulta = mysql_query( $sql, $con );
...
function formulario( $vars )
{
	global $con;
	$desvio = "";
	$sql = "Select * from `" . $vars['tabla'] . "` where id like " . $vars['registro'];
	$consulta = mysql_query( $sql, $con );
...
function tipoCampo( $campo, $tabla, $valor, $opcion, $orden )
{
	...
	switch ($resultado['tipo']) {
            ...
            if (($tabla == 'z_sercont') &&
             ($resultado['campoo'] == 'valor')) {
                $cadena = "<div id='tipo_teleco'><input type='text' size='" .
                 $resultado['size'] . "' id='" . $resultado['variable'] . "' name='" .
                 $resultado['campoo'] . "' value='" . $valor .
        ...
        case "select":
            ...
            if ($tabla == 'z_sercont') {
                $cadena = "<select id='" .
                 $resultado['variable'] . "' name='" . $resultado['campoo'] .
                 "' tabindex='" . $i . "' onchange='muestra_campo()'>";
            } else {
...
function nuevo( $vars )
{
	...
	for ( $i=1, $j=0; $i<=$numero_campos-1; $i++, $j++ ) {
		...
		tipoCampo( mysql_field_name( $consulta, $i ), $resultado[0], '', 'nuevo', $i ) . 
		"</td>";
	}
	$cadena .= "</tr>";
...
function subformulario( $vars )
{
	...
	if ( isset( $vars['tabla'] ) ) {
		switch ( $vars['tabla'] ) {
			case "pproveedores":
			    ...
			    on c.id = t.idemp where t.id like " . $vars['registro'];
			break;
			default:
	...
	switch ( $resultado[1] ) {
		...
		case "proveedores":
            ...
            $code = "";
        break;
        default: $sql = "Select Nombre from clientes where 
        	id like " . $registro;
...
function subform( $sql, $tabla, $registro, $marcado )
{
	...
	for ( $i=2; $i<=$numero_campos-1; $i++ ) {
		if ( $tipo == "nuevo" ) {
			...
			bgcolor='#eeeeee'>" . 
			tipoCampo( mysql_field_name( $consulta, $i ), $tabla, "", $tipo, $i ) 
			. "</td></tr>";
			...
			bgcolor='#eeeeee'>" . 
			tipoCampo( 
			    mysql_field_name( $consulta, $i ), $tabla, $resultado[$i], $tipo, $i 
			) ."</td></tr>";
...
function sublist( $sql, $tabla )
{
	...
	while ( true == ($resultado = mysql_fetch_array( $consulta ) ) ) {
		...
		for ( $i=2; $i<=$totcampos-1; $i++ ) {
		    ...
		    ucfirst( compruebaCheck(
		        $tabla, mysql_field_name( $consulta, $i ), $resultado[$i] 
		        )
		    ) . "holaquetal</td>";
		}
		$cadena .= "</tr>";
...
function serviciosFijos( $cliente )
{
	...
	<input type='button' class='agregar' onclick='frm_srv_fijo(" . $cliente . ")' />
	</th></tr>";
...
function frmSrvFijos( $vars )
{
	...
	if ( isset( $vars['cliente'] ) ) {
		...
		id='importe' size='8'value='" . $resultado2['Imp_Euro'] . "'/>&euro;</td>";
		...
		size='2'value='" . $resultado2['iva'] . "'/></td></tr>";
		$cadena .= "<tr><th valign='top'>Observaciones:</th><td>
...
function actualizaSrvFijo( $vars )
{
	...
	$sql = "Update `tarifa_cliente` set `Servicio`='" . $vars['servicio'] . "',
	 `Imp_Euro`='" . $vars['importe'] . "', `iva`='" . $vars['iva'] . "', 
