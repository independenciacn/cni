...
foreach($hoy as $linea)
{
    foreach($linea as $lin)
        $cumple_dia[]= array(utf8_encode($lin[nombre])." ".utf8_encode($lin[apellidos]),utf8_encode($lin[Nombre]));
}
//Parte del mes
    //Empleados Mes
...
{
    foreach($linea as $lin)
        $cumple_mes[]= array($cumple->cambiaf($lin[cumple]),utf8_encode($lin[nombre])." ".utf8_encode($lin[apellidos]),utf8_encode($lin[Nombre]));
}
...
$cadena .= "<tr><td colspan='3' align='left'><span class='boton' onclick='cierralo()' onkeypress='cierralo()'>[X] Cerrar</span></td></tr>";
...
if(count($cumple_dia)!=0)
{
    ...
    {
    $cadena .="<tr><td class='".$cumple->clase($k)."'></td><td class='".$cumple->clase($k)."'>".$dia_de_hoy[0]."</td><td class='".$cumple->clase($k)."'>".$dia_de_hoy[1]."</td></tr>";
	$k++;
    }
}
else
...
if(count($cumple_mes)!=0)
{
    ...
    {
        $cadena .="<tr><td class='".$cumple->clase($k)."'>".$dia_de_hoy[0]."</td><td class='".$cumple->clase($k)."'>".$dia_de_hoy[1]."</td><td class='".$cumple->clase($k)."'>".$dia_de_hoy[2]."</td></tr>";
        $k++;
    }
}
else
