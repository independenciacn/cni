...
if(isset($_SESSION[usuario]))
{
	if(isset($_POST[opcion]))
	{
		switch($_POST[opcion])
		{
			case(0):$cadena=formulario($_POST);break;//Generamos el formulario
			case(1):$cadena=respuesta($_POST);break;//Generamos la respuesta
			case(2):$cadena=comparativas($_POST);break;//Genera la pantalla de comparativa
		}
		echo $cadena;
	}
	else
 ...
 */
 function clientes()
 {
	...
	include("../inc/variables.php");
	$sql = "Select id,Nombre from clientes order by Nombre"; //mostramos tambien los inactivos
	$consulta = @mysql_db_query($dbname,$sql,$con);
	...
	while($resultado = @mysql_fetch_array($consulta))
	{
		...
		if($cliente == $resultado[0])
			...
			$marcado = "";
		$form .= "<option value='$resultado[0]' $marcado >".utf8_encode($resultado[1])." $resultado[2]</option>";
	}
	$form .= "</select>";
	return $form;
}
 ...
 */
 function categorias()
 {
	...
	while($resultado = @mysql_fetch_array($consulta))
	{
		$form .= "<option value='".utf8_encode($resultado[0])."' >".utf8_encode($resultado[0])."</option>";
	}
	$form.="</select>";
	return $form;
 }
...
function dias($modo)
{
	switch($modo)
	{
		case 0:$tipo = "dia";break;
		case 1:$tipo = "diaf";break;
		case 2:$tipo = "rdia";break;
		case 3:$tipo = "rdiaf";break;
	}
...
function mes($modo)
{
	switch($modo)
	{
		case 0:$tipo = "mes";break;
		case 1:$tipo = "mesf";break;
		case 2:$tipo = "rmes";break;
		case 3:$tipo = "rmesf";break;
	}
	$select.="<select id='".$tipo."' name='".$tipo."'>";
...
function anyo($modo)
{
	switch($modo)
	{
		case 0:$tipo = "ano";break;
		case 1:$tipo = "anof";break;
		case 2:$tipo = "rano";break;
		case 3:$tipo = "ranof";break;
	}
	$select.="<select id='".$tipo."' name='".$tipo."'>";
 ...
 */
 function servicios()
 {
	...
	$sql = "Select trim(servicio) from historico group by trim(servicio) order by trim(servicio)";
	$consulta = @mysql_db_query($dbname,$sql,$con);
 ...
 */ 
/*
 * Function que genera el formulario para la consulta por cliente
 */
function formulario($vars)
{
	...
	$cadena="<form name='consulta' id='consulta' method='post' onsubmit='procesa();return false'>";
	$cadena.="<input type='hidden' name='formu' id='formu' value='".$vars[form]."'>";
	switch($vars[form])
	{
		//Entre Fechas por cliente
		case(0):$cadena.=clientes()."Inicio:".fecha(0)."Fin:".fecha(1);break;
		//Entre Fechas por categoria
		case(1):$cadena.=categorias()."Inicio:".fecha(0)."Fin:".fecha(1);break;
		//Entre Fechas por servicios
		case(2):$cadena.=servicios()."Inicio:".fecha(0)."Fin:".fecha(1);break;
		//Entre Fechas por cliente/servicios
		case(3):$cadena.=clientes().servicios()."<br/>Inicio:".fecha(0)."Fin:".fecha(1);break;
		//Entre Fechas por categoria/servicios
		case(4):$cadena.=categorias().servicios()."<br/>Inicio:".fecha(0)."Fin:".fecha(1);break;
		//Servicios por volumen de facturacion entre fechas
		case(5):$cadena.=servicios()."<br/>Inicio:".fecha(0)."Fin:".fecha(1);break;
		//Clientes por volumen de facturacion entre fechas
		case(6):$cadena.=clientes()."<br/>Inicio:".fecha(0)."Fin:".fecha(1);break;
		//Comparativas
		case(7):$cadena.= comparativas($vars);break;
	}
	if($vars[form]!=7)
	{
		$cadena.="<br /><input type='radio' name='tipo' value='acumulado' checked='checked'> Acumulado";
		$cadena.="<input type='radio' name='tipo' value='detallado'> Detallado";
		$cadena.="<input type='radio' name='tipo' value='comparativa'>Comparativa";
		$cadena.="&nbsp;&nbsp;Limite Comparativa:";
...
function comparativas($vars)
{
	if(!isset($vars['tipo']))
	{
		...
		$cadena.="Comparacion de:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<select name='tipo_comparativa' id='tipo_comparativa' onchange='comparativa()'>";
	...
	else
	{
		switch($vars['tipo'])
		{
			case(1):$cadena="Seleccione Cliente:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".clientes().servicios();break;
	...
	Inicio Rango:<input type='text' readonly size='10' id='fecha_inicio_b' name='fecha_inicio_b' />
...
function respuesta($vars)
{
	switch($vars[formu])
	{
		case(0):$vars[titulo]="Consumo mensual y acumulado entre fechas por cliente";$cadena=consulta($vars);break;
		case(1):$vars[titulo]="Consumo mensual y acumulado entre fechas por categoria";$cadena=consulta($vars);break;
		case(2):$vars[titulo]="Consumo mensual y acumulado entre fechas por servicios";$cadena=consulta($vars);break;
		case(3):$vars[titulo]="Consumo mensual y acumulados entre fechas por cliente/servicio";$cadena=consulta($vars);break;
		case(4):$vars[titulo]="Consumo mensual y acumulado entre fechas por categoria/servicio";$cadena=consulta($vars);break;
		case(5):$vars[titulo]="Servicios mas facturados";$cadena=consulta($vars);break;
		case(6):$vars[titulo]="Clientes con mas facturacion";$cadena=consulta($vars);break;
		case(7):$vars[titulo]="Comparativas";$cadena=consulta($vars);break;
	}
	return $cadena;
}
/*
...
//Consultas entre fechas de clientes	
	if($vars[formu]==0)
	{
		if($vars[tipo]=="acumulado")
			$sql="SELECT trim(h.servicio) as Servicio, sum(h.cantidad) as Unidades, sum(h.cantidad*h.unitario) as Importe, sum(h.cantidad*h.unitario*h.iva/100) Iva, sum(h.cantidad*h.unitario+h.cantidad*h.unitario*h.iva/100) as Total
			FROM `historico` as h 
			INNER JOIN `regfacturas` AS c on h.factura = c.codigo INNER JOIN `clientes` AS l ON c.id_cliente = l.Id
			WHERE c.id_cliente like '".$vars[cliente]."' ".$con_fecha." GROUP BY h.servicio";
		else
			$sql="SELECT c.fecha, trim(d.servicio) as Servicio, trim(d.obs) as Observaciones, d.cantidad as Unidades, d.unitario as Importe, d.iva, ((d.unitario*d.cantidad) + (d.unitario*d.cantidad)*d.iva/100) as Total
			FROM `historico` AS d
			INNER JOIN `regfacturas` AS c on c.`codigo` = d.`factura` INNER JOIN `clientes` AS s ON c.id_cliente = s.Id
...
//Consultas entre fechas de Categorias	
	if($vars[formu]==1)
	{
		if($vars[tipo]=="acumulado")
            ...
            and e.Categoria like '".utf8_decode($vars[categoria])."'
		...
		*/
            ...
            where  ((e.salida >= c.fecha or e.salida like '0000-00-00')
            and e.entrada <= c.fecha 
            and e.categoria like '".utf8_decode($vars[categoria])."' ".$con_fecha.")
            order by c.fecha, s.Nombre";
         

        
        $subtitulo = utf8_decode($vars[categoria]);
	}
//Consumo mensual y acumulado entre fechas por servicios,control de almacenaje y fijos
	if($vars[formu]==2)
	{
		if($vars[tipo]=="acumulado")
			$sql="SELECT sum(h.cantidad) as Unidades, sum(h.cantidad*h.unitario) as Importe, sum(h.cantidad*h.unitario*h.iva/100) Iva, sum(h.cantidad*h.unitario+h.cantidad*h.unitario*h.iva/100) as Total
			FROM `historico` as h 
			INNER JOIN `regfacturas` AS c on h.factura = c.codigo INNER JOIN `clientes` AS l ON c.id_cliente = l.Id
			WHERE trim(h.servicio) like '".$vars[servicios]."' ".$con_fecha." GROUP BY h.servicio";
		else
			$sql="SELECT c.fecha, s.nombre as Cliente, d.obs as Observaciones, d.cantidad as Unidades, d.unitario as Importe, d.iva, ((d.unitario*d.cantidad) + (d.unitario*d.cantidad)*d.iva/100) as Total
			FROM `historico` AS d
			INNER JOIN `regfacturas` AS c on c.`codigo` = d.`factura` INNER JOIN `clientes` AS s ON c.id_cliente = s.Id
			WHERE trim(d.servicio) LIKE '".$vars[servicios]."' ".$con_fecha." order by c.fecha";
		$subtitulo = utf8_decode($vars[servicios]);
	}
//Consumo mensual y acumulados entre fechas por cliente/servicio
	if($vars[formu]==3)
	{
		if($vars[tipo]=="acumulado")
			...
			FROM `historico` as h 
			INNER JOIN `regfacturas` AS c on h.factura = c.codigo INNER JOIN `clientes` AS l ON c.id_cliente = l.Id
			WHERE trim(h.servicio) like '".$vars[servicios]."' ".$con_fecha." and c.id_cliente LIKE '$vars[cliente]' GROUP BY h.servicio";
			else
			$sql="SELECT c.fecha, d.obs as Observaciones, d.cantidad as Unidades, d.unitario as Importe, d.iva, ((d.unitario*d.cantidad) + (d.unitario*d.cantidad)*d.iva/100) as Total
			FROM `historico` AS d
			INNER JOIN `regfacturas` AS c on c.`codigo` = d.`factura` INNER JOIN `clientes` AS s ON c.id_cliente = s.Id
			WHERE trim(d.servicio) LIKE '".$vars[servicios]."' ".$con_fecha." and c.id_cliente LIKE '$vars[cliente]' order by c.fecha";
		$subtitulo = nombre_cliente($vars[cliente])." / ".utf8_decode($vars[servicios]);
	}
//Consumo mensual y acumulado entre fechas por categoria/servicio
	if($vars[formu]==4)
	{
		if($vars[tipo]=="acumulado")
			...
			FROM `historico` as h 
			INNER JOIN `regfacturas` AS c on h.factura = c.codigo INNER JOIN `clientes` AS l ON c.id_cliente = l.Id
			WHERE trim(h.servicio) like '".$vars[servicios]."' ".$con_fecha." and l.Categoria LIKE '".utf8_decode($vars[categoria])."' GROUP BY h.servicio";
		else
			$sql="SELECT c.fecha, s.Nombre as Cliente, d.obs as Observaciones, d.cantidad as Unidades, d.unitario as Importe, d.iva, ((d.unitario*d.cantidad) + (d.unitario*d.cantidad)*d.iva/100) as Total
			FROM `historico` AS d
			INNER JOIN `regfacturas` AS c on c.`codigo` = d.`factura` INNER JOIN `clientes` AS s ON c.id_cliente = s.Id
			WHERE trim(d.servicio) LIKE '".$vars[servicios]."' ".$con_fecha." and s.Categoria LIKE '".utf8_decode($vars[categoria])."' order by c.fecha";
		$subtitulo = nombre_cliente($vars[categoria])." / ".utf8_decode($vars[servicios]);
	}
//Servicios por volumen de facturacion + facturados
	if($vars[formu]==5)
	{
		if($vars[tipo]=="acumulado")
			...
			$sql="SELECT c.fecha, trim(d.servicio) as Servicio, trim(d.obs) as Observaciones, s.Nombre as Cliente, d.cantidad as Unidades, d.unitario as Importe, d.iva, ((d.unitario*d.cantidad) + (d.unitario*d.cantidad)*d.iva/100) as Total
			FROM `historico` AS d
			INNER JOIN `regfacturas` AS c on c.`codigo` = d.`factura` INNER JOIN `clientes` AS s ON c.id_cliente = s.Id
			...
			{
				if($vars[servicios]!="0")
					$filtra_servicio = " and trim(h.servicio) like trim('$vars[servicios]') ";
				else
					$filtra_servicio = "  ";
				$rango=array_de_rangos($vars);
				foreach($rango as $rangillo)
				$esql[]="SELECT trim(h.servicio) as Servicio,sum(h.cantidad*h.unitario+h.cantidad*h.unitario*h.iva/100) as Total
				FROM `historico` as h 
				INNER JOIN `regfacturas` AS c on h.factura = c.codigo INNER JOIN `clientes` AS l ON c.id_cliente = l.Id
				".$rangillo." ".$filtra_servicio." GROUP BY trim(h.servicio) order by Total desc ".$limite;
	...
	if($vars[formu]==6)
	{
		...
		".$con_fecha." GROUP BY l.Nombre order by Total desc";
			...
			if($vars[tipo]=="detallado")
		$sql="SELECT c.fecha, s.Nombre as Cliente, trim(d.servicio) as Servicio, trim(d.obs) as Observaciones,  d.cantidad as Unidades, d.unitario as Importe, d.iva, ((d.unitario*d.cantidad) + (d.unitario*d.cantidad)*d.iva/100) as Total
		FROM `historico` AS d
		INNER JOIN `regfacturas` AS c on c.`codigo` = d.`factura` INNER JOIN `clientes` AS s ON c.id_cliente = s.Id
		".$con_fecha." order by c.fecha";
			else /*Comparativa entre rangos*/
			{
				...
				$rango=array_de_rangos($vars);
				foreach($rango as $rangillo)
					$esql[]="SELECT l.Nombre as Cliente, sum(h.cantidad*h.unitario+h.cantidad*h.unitario*h.iva/100) as Total
		FROM `historico` as h 
		INNER JOIN `regfacturas` AS c on h.factura = c.codigo INNER JOIN `clientes` AS l ON c.id_cliente = l.Id
		".$rangillo." ".$filtra_cliente." GROUP BY l.Nombre order by Total desc ".$limite;
			}
		$subtitulo = "";
	}
	...
	if($vars[formu]==7)
	{
		switch($vars[tipo_comparativa])
		{
			//Clientes
			case 1:
			$sql="SELECT l.Nombre as Cliente, sum(h.cantidad*h.unitario+h.cantidad*h.unitario*h.iva/100) as Total
		FROM `historico` as h 
		INNER JOIN `regfacturas` AS c on h.factura = c.codigo INNER JOIN `clientes` AS l ON c.id_cliente = l.Id";
			if($vars[cliente]!=0)
			{
			...
			//Servicios
			case 2:
			$sql="SELECT trim(h.servicio) as Servicio, sum(h.cantidad*h.unitario+h.cantidad*h.unitario*h.iva/100) as Total
				FROM `historico` as h 
				INNER JOIN `regfacturas` AS c on h.factura = c.codigo INNER JOIN `clientes` AS l ON c.id_cliente = l.Id";
			if($vars[servicios]!="0")
			{
			...
			if($vars[categoria]!="0")
			{
				$filtro = " and l.Categoria like '".utf8_decode($vars[categoria])."' ";
		...
		{
			$rango.="-1";
			$esql[]=$sql." where year('$rango') like year(c.Fecha) and month('$rango') like month(c.fecha)  ".$filtro.$grupo;
		}
		if($vars[fecha_inicio_b]!='' && $vars[fecha_fin_b]!='')
		{
			...
			{
				$rango.="-1";
				$esql2[]=$sql." where year('$rango') like year(c.Fecha) and month('$rango') like month(c.fecha)  ".$filtro.$grupo;
			}
		}
	}
...
if($vars[formu]!=7)
{
	...
	$_SESSION[titulo]=$vars[titulo]." - ".utf8_encode($subtitulo);
	if(is_array($esql))
	{
	...
	$cadena.="<br/><span class='boton' onclick='window.open(\"print.php?sql=".urlencode($sql)."\",\"_self\")'>Imprimir</span>";	
}
else
{
	if(is_array($esql))
...
function genera_consultas($inicio,$fin)
{
	...
	$sql = "Select year(fecha) from regfacturas where (fecha>='$inicio' and fecha<='$fin') group by year(fecha) order by year(fecha)";
	//echo $sql;
	$consulta = @mysql_db_query($dbname,$sql,$con);
	while($resultado = @mysql_fetch_array($consulta))
	...
	(fecha>='$inicio' and fecha<='$fin' and year(fecha) like $anyo) group by month(fecha) order by month(fecha)";
	$consulta = @mysql_db_query($dbname,$sql,$con);
	while($resultado = @mysql_fetch_array($consulta))
 ...
 function array_de_rangos($vars)
 {
	...
	if($vars[ano]==0 && $vars[anof]==0)
	{
		...
		if($resultado[0]>='2008')
			...
			$cadena[]= " where month(c.Fecha)>=8 and year(c.Fecha) like $resultado[0]";
	...
	if($vars[ano]!=0 && $vars[anof]!=0)
	{
		...
		if($i>=2008)
		$sql = "Select month(fecha) from regfacturas where year(fecha) like $i group by month(fecha) ";
		else
		$sql = "Select month(fecha) from regfacturas where year(fecha) like $i and month(fecha) >= 8 group by month(fecha) ";
		$consulta = @mysql_db_query($dbname,$sql,$con);
		while($resultado=@mysql_fetch_array($consulta))
			$cadena[]= " where month(c.Fecha) like $resultado[0] and year(fecha) like $i";
		}
	}
	return $cadena;
 }
...
while($resultado = mysql_fetch_array($consulta))
{
    ...
    if(isset($resultado["Nombre"]) && isset($resultado["fecha"]))
    {
            ...
            if(((utf8_encode($resultado["Nombre"])!=$aux)&&($aux != " "))||(($resultado["fecha"]!=$aux2)&&($aux != " ")))
            {
                $cadena.="<tr><th colspan='8'>Total ".$aux." ".cambiaf($aux2)."</th><th>".round($minitot,2)."&euro;</th></tr>";
                $pal_final = $cadena;
                $aux2 = $resultado["fecha"];
                $aux = utf8_encode($resultado["Nombre"]);
                $minitot = 0;
            }
            else
                if($aux == " ")
		...
		switch(mysql_field_type($consulta,$i))
		{
			case "string":
						if(mysql_field_name($consulta,$i)=="Servicio")
							...
							$campo = utf8_encode($resultado[$i]);break;
...
function genera_la_tabla_chunga($sql,$vars,$subtitulo)
{
	...
	$cadena.="<table class='tabla' style='font-size:10px'>";
	...
	for($j=1;$j<=count($dato);$j++)
	{
		...
		//Aqui recorremos 10 veces la tabla que almacena las claves;
			...
			{
				if($vars[formu]==6)
					$cadena.="<td class='par' valign='top'><b>".utf8_encode($claves[$l+1])."</b></td>";
				else
					$cadena.="<td class='par' valign='top'><b>".utf8_encode($claves[$l+1])."</b></td>";
			}
		$cadena.="</tr><tr>";
