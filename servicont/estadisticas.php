 ...
 function clientes()
 {
	...
	while($resultado = @mysql_fetch_array($consulta))
	{
		...
		if($cliente == $resultado[0])
			...
			$marcado = "";
		$form .= "<option value='$resultado[0]' $marcado >".$resultado[1]." $resultado[2]</option>";
	}
	$form .= "</select>";
	return $form;
}
...
//Consultas entre fechas de Categorias	
	if($vars[formu]==1)
	{
		if($vars[tipo]=="acumulado")
			...
			WHERE s.Categoria like '".$vars[categoria]."' ".$con_fecha." order by c.fecha";
		*/
...
//Consumo mensual y acumulado entre fechas por categoria/servicio
	if($vars[formu]==4)
	{
		if($vars[tipo]=="acumulado")
			...
			FROM `historico` as h 
			INNER JOIN `regfacturas` AS c on h.factura = c.codigo INNER JOIN `clientes` AS l ON c.id_cliente = l.Id
			WHERE trim(h.servicio) like '".$vars[servicios]."' ".$con_fecha." and l.Categoria LIKE '".$vars[categoria]."' GROUP BY h.servicio";
		else
			$sql="SELECT c.fecha, s.Nombre as Cliente, d.obs as Observaciones, d.cantidad as Unidades, d.unitario as Importe, d.iva, ((d.unitario*d.cantidad) + (d.unitario*d.cantidad)*d.iva/100) as Total
			FROM `historico` AS d
			INNER JOIN `regfacturas` AS c on c.`codigo` = d.`factura` INNER JOIN `clientes` AS s ON c.id_cliente = s.Id
			WHERE trim(d.servicio) LIKE '".$vars[servicios]."' ".$con_fecha." and s.Categoria LIKE '".$vars[categoria]."' order by c.fecha";
...
$cadena.="<tr><th></th><th colspan='".mysql_num_fields($consulta)."'>".$vars[titulo]." - ".$subtitulo."</th></tr>";
...
while($resultado = mysql_fetch_array($consulta))
{
    ...
    if(isset($resultado["Nombre"]) && isset($resultado["fecha"]))
    {
            ...
            if((($resultado["Nombre"]!=$aux)&&($aux != " "))||(($resultado["fecha"]!=$aux2)&&($aux != " ")))
            {
