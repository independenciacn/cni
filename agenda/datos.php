...
if ( isset($_SESSION['usuario']) && isset( $_POST['opcion'] ) ) {
    sanitize( $_POST );
} else {
    notFound();
}
...
switch($_POST['opcion'])
{
	case 0:$respuesta=formularioDespacho($_POST);break;
	case 1:$respuesta=cuca($_POST);break;
	case 2:$respuesta=dameNombreCliente($_POST);break;
	case 3:$respuesta=informacionDespacho($_POST);break;
...
function cuca($vars)
{
	...
	else
	{
		...
		{
			$muestra .="<span class='lbl_clientes' onclick='marca(".$resultado[0].")' onmouseout='quitar_color(".$resultado[0].")' onmouseover='cambia_color(".$resultado[0].")'><p id='linea_".$resultado[0]."'>".eregi_replace($vars[texto],"<b><u>".strtoupper($vars[texto])."</u></b>",$resultado[1])."</span></p>";
		}
	}
	return $muestra;
}
 ...
 */
function informacionDespacho( $vars )
{
	//include("../inc/variables.php");
	...
	if($vars['tipo']==0)
	{
		...
		$datosCliente = explode(";",dameNombreCliente($cliente));
		$cadena.="<form id='ficha_cliente' method='post' action=''>";
		$cadena.="<div class='seccion'>Cliente:</div><p>".$datosCliente[1]."</p>";
	//Codigo Negocio
		$cadena.=comunicaciones($datosCliente[0],"Codigo Negocio");
	//Telefonos
		$cadena.=comunicaciones($datosCliente[0],"Telefono");
	//Adsl
		$cadena.=comunicaciones($datosCliente[0],"Adsl");
	//Fax
		$cadena.=comunicaciones($datosCliente[0],"Fax");
	//Direcciones IP
		$cadena.=comunicaciones($datosCliente[0],"Direccion IP");
	//Codigo Fotocopias
		$cadena.=comunicaciones($datosCliente[0],"Codigo Fotocopias");
	//Codigo Fotocopias
		$cadena.=comunicaciones($datosCliente[0],"Codigo Fotocopias Autoservicio");
	//Empleados de la empresa
		$cadena.= empleados($datosCliente[0]);
		//Parte de las observaciones
		$sql = "Select id_cliente,obs,id,despacho,conformidad,
		repeticion,hinc,hfin,finc,ffin from agenda where 
		id like ". $vars['despacho'];
		echo $sql;
		$resultado = consultaUnica($sql);
...
{
	$sql = "select * from z_sercont where idemp like ". $cliente ." 
	and servicio like '".$seccion."'";
	$resultados = consultaGenerica( $sql );
	$html = "";
	if( count($resultados) > 0) {
		$html.="<div class='seccion'>".$seccion.":</div>";
		foreach( $resultados as $resultado) {
			$html.="<p>".$resultado['valor'];
			if($resultado['boca']!="")
				$html.="- EXT:".$resultado['boca'];
			$html.="</p>";
		}
	}
	return $html;
...
function empleados( $cliente )
{
	...
	//echo $sql;//Depuracion
	if ( count( $resultados ) > 0) {
		$html = "<div class='seccion'>Empleados:</div>";
		foreach( $resultados as $resultado ) {
			$html.="<p>".$resultado['nombre']." ".$resultado['apellidos'];
			if($resultado['email']!="") {
			    $html.="&nbsp;<a href=mailto:".$resultado['email']." 
			    alt=".$resultado['email'].">
			    <img class='tipTip' src='../estilo/iconos/mail.png' title='Enviar e-mail'/></a>";
			}
			$html.="</p>";
			return $html;
		}
...
&nbsp;&nbsp;&nbsp;&nbsp;</div><input type='hidden' value='".$vars[despacho]."' id='codigo_despacho' />";
	if(@mysql_numrows($consulta)!=0)
	{
		...
		//CASO RESERVAS
		if($resultado[otro]!="")
			$respuesta.="<tr><td colspan='2'>".$resultado[otro]."</td></tr>";
		else
			$respuesta.="<tr><td colspan='2'>".nombre_cliente($resultado[id_cliente])."</td></tr>";
