...
if($_GET[mes] <= 9)
	...
	$cadena .= "<table cellpadding='0px' cellspacing='0px' width='100%' border='1' id='tabloide'>";
	$cadena .= "<tr bgcolor='#bbbbbb'><th colspan='7'>Listado de Servicios de ".codifica($cliente_servicios[1])." en ".$mes_servicios." de ".$_GET[anyo]."</th></tr>";
	$cadena .= "<tr bgcolor='#bbbbbb'><th>Fecha</th><th>Servicio</th><th>Cantidad</th><th>Precio Unidad</th><th>Importe</th><th>Iva</th><th>Total</th></tr>";
...
//acumulados
		...
		<td ".$color."><p class='texto'>".ucfirst(codifica($resultado[2]))." ".ucfirst(codifica($resultado[6]))."</p></td>
		<td ".$color." align='right'>".number_format("1",2,',','.')."&nbsp;</td>
		<td ".$color." align='right'>".number_format($resultado[4],2,',','.')."&euro;&nbsp;</td>
		<td ".$color." align='right'>".number_format($resultado[4],2,',','.')."&euro;&nbsp;</td>
		<td ".$color." align='right'>".$resultado[5]."%&nbsp;</td>
		<td ".$color." align='right'>".number_format($subtotal,2,',','.')."&euro;</td></tr>";
		$cantidad++;
	}
...
if (($cliente == '295') || ($cliente == '301'))
{
		...
		on c.`Id Pedido` = d.`Id Pedido` where c.Cliente like $cliente and 
...
//fin acumulados
		...
		$cadena.= "<tr><td ".$color."><p class='texto'>".$resultado[2]."</p></td>
		<td ".$color."><p class='texto'>".ucfirst($resultado[0])." ".codifica(ucfirst($resultado[7]))."</p></td>
		<td ".$color." align='right'>".number_format($resultado[1],2,',','.')."&nbsp;</td>
		<td ".$color." align='right'>".number_format($resultado[3],2,',','.')."&euro;&nbsp;</td>
		<td ".$color." align='right'>".number_format($resultado[4],2,',','.')."&euro;&nbsp;</td>
		<td ".$color." align='right'>".$resultado[5]."%&nbsp;</td>
		<td ".$color." align='right'>".number_format($subtotal,2,',','.')."&euro;&nbsp;</td></tr>";
	}
	$cadena.= "<tr><th>&nbsp;</th>
	<th>&nbsp;</th>
	<th>&nbsp;".number_format($cantidad,2,',','.')."</th><th>&nbsp;</th><th>&nbsp;".number_format($parcial,2,',','.')."&euro;</th><th>&nbsp;</th><th>".number_format($total,2,',','.')."&euro;</th></tr>";
		$cadena.= "</table>";
echo $cadena;
}
else
