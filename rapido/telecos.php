...
function agrega_telefono($cliente,$telefono)
{
	...
	{	
		$sql = "Insert into z_sercont (servicio,idemp,valor) values ('telefono','$cliente','$telefono')";
		if ($consulta = mysql_db_query($dbname,$sql,$con))
...
function agrega_adsl($cliente,$adsl)
{
	...
	if ($previa == "#00ff00") //el numero es correcto
	{	
		$sql = "Insert into z_sercont (servicio,idemp,valor) values ('adsl','$cliente','$adsl')";
		if ($consulta = mysql_db_query($dbname,$sql,$con))
...
function agrega_fax($cliente,$fax)
{
	...
	if ($previa == "#00ff00") //el numero es correcto
	{	
		$sql = "Insert into z_sercont (servicio,idemp,valor) values ('fax','$cliente','$fax')";
		if ($consulta = mysql_db_query($dbname,$sql,$con))
...
function agrega_fotocopias($cliente,$fotocopias)
{
	...
	if ($previa == "#00ff00") //el numero es correcto
	{	
		$sql = "Insert into z_sercont (servicio,idemp,valor) values ('fotocopias','$cliente','$fotocopias')";
		if ($consulta = mysql_db_query($dbname,$sql,$con))
...
function agrega_afotocopias($cliente,$afotocopias)
{
	...
	if ($previa == "#00ff00") //el numero es correcto
	{	
		$sql = "Insert into z_sercont (servicio,idemp,valor) values ('afotocopias','$cliente','$afotocopias')";
		if ($consulta = mysql_db_query($dbname,$sql,$con))
			$tabla = "<span class='avisok'>Codigo de fotocopias Autoservicio asignado al cliente</span>";
		else
			$tabla = "<span class='avisonok'>No se ha asignado el codigo de fotocopias Autoservicio al cliente</span>";
	}
	else
		$tabla = "<span class='avisonok'>No se ha asignado el codigo de fotocopias Autoservicio al cliente</span>";
	return $tabla;
...
function agrega_negocio($cliente,$negocio)
{
	...
	if ($previa == "#00ff00") //el numero es correcto
	{	
		$sql = "Insert into z_sercont (servicio,idemp,valor) values ('negocio','$cliente','$negocio')";
		if ($consulta = mysql_db_query($dbname,$sql,$con))
...
//Direcciones IP******************************************************************************************/
	...
	if(mysql_numrows($consulta) >= 1)
		while ($resultado = mysql_fetch_array($consulta))
			$lista_ips .= "<tr><td>172.26.0.".$resultado[3]."</td><td><span class='boton' onclick='borra_servicio(".$resultado[0].")'><img src='../imagenes/borrar.png' alt='Borrar' /></span></td></tr>";
	else
...
//Telefonos******************************************************************************************/
	...
	if(mysql_numrows($consulta) >= 1)
		while ($resultado = mysql_fetch_array($consulta))
			$lista_telefonos .= "<tr><td>".$resultado[3]."</td><td><span class='boton' onclick='borra_servicio(".$resultado[0].")'><img src='../imagenes/borrar.png' alt='Borrar' /></span></td></tr>";
	else
...
//Adsl******************************************************************************************/
	...
	if(mysql_numrows($consulta) >= 1)
		while ($resultado = mysql_fetch_array($consulta))
			$lista_adsl .= "<tr><td>".$resultado[3]."</td><td><span class='boton' onclick='borra_servicio(".$resultado[0].")'><img src='../imagenes/borrar.png' alt='Borrar' /></span></td></tr>";
	else
...
//Fax******************************************************************************************/
	...
	if(mysql_numrows($consulta) >= 1)
		while ($resultado = mysql_fetch_array($consulta))
			$lista_fax .= "<tr><td>".$resultado[3]."</td><td><span class='boton' onclick='borra_servicio(".$resultado[0].")'><img src='../imagenes/borrar.png' alt='Borrar' /></span></td></tr>";
	else
...
//Codigos de fotocopias******************************************************************************************/
	...
	if(mysql_numrows($consulta) >= 1)
		while ($resultado = mysql_fetch_array($consulta))
			$lista_fotocopias .= "<tr><td>".$resultado[3]."</td><td><span class='boton' onclick='borra_servicio(".$resultado[0].")'><img src='../imagenes/borrar.png' alt='Borrar' /></span></td></tr>";
	else
...
//Codigos de fotocopias autoservicio******************************************************************************************/
	...
	if(mysql_numrows($consulta) >= 1)
		while ($resultado = mysql_fetch_array($consulta))
			$lista_afotocopias .= "<tr><td>".$resultado[3]."</td><td><span class='boton' onclick='borra_servicio(".$resultado[0].")'><img src='../imagenes/borrar.png' alt='Borrar' /></span></td></tr>";
	else
...
//Codigos de negocio******************************************************************************************/
	...
	if(mysql_numrows($consulta) >= 1)
		while ($resultado = mysql_fetch_array($consulta))
			$lista_negocios .= "<tr><td>".$resultado[3]."</td><td><span class='boton' onclick='borra_servicio(".$resultado[0].")'><img src='../imagenes/borrar.png' alt='Borrar' /></span></td></tr>";
	else
