...
/*global $, Ajax, Form, $F, Calendar, confirm*/
 ...
 * @type {String}
 */
var urlDatos = "datos.php";
/**
 * Imagen que muestra el cargando mientras carga datos
 *
 * @type {String}
 */
var imgCarga = "<img src='../estilo/custom-theme/images/ajax-loader.gif' " +
    "alt='cargando' />";
/**
 ...
 *
 * @param {Object} pars Parametros peticion
 ...
 */
var procesaAjax = function (pars, divName, divPrecarga, callback, params, url) {
    "use strict";
    var options, myAjax, urlAction;
    urlAction = (!url) ? urlDatos : url;
    options = {
		method: 'post',
		parameters: pars,
		onSuccess: function gen(t) {
            if (divName) {
                $(divName).innerHTML = t.responseText;
            }
            if (callback) {
				callback(params, t);
			}
	...
	myAjax = new Ajax.Request(urlDatos, options);
};
/**
...
var buscaCliente = function () {
	...
	procesaAjax(pars, 'listadoClientes', false, false, false, false);
};
/**
 ...
 * @param t
 */
var verServiciosContratados = function (idCliente, t) {
    "use strict";
    var pars, cliente;
    cliente = (!idCliente) ? $F('idCliente') : idCliente;
    pars = "opcion=3&cliente=" + cliente +
        "&mes=" + $('meses').value + "&anyo=" + $('anyo').value;
    procesaAjax(pars, 'tabla', 'tabla', false, false, false);
};
/**
 * Una vez marcado el cliente procesa los datos
 *
 * @param params
 * @param t
 */
var procesaDatosRespuesta = function (params, t) {
    "use strict";
    var datos = t.responseText.split('#');
    $('idCliente').value = datos[0];
    $('cliente').value = datos[1];
    $('listadoClientes').innerHTML = "";
    verServiciosContratados(datos[0], t);
};
/**
...
var marca = function (idCliente) {
    ...
    procesaAjax(pars, false, false, procesaDatosRespuesta, false, false);
};


/**
 * Procesa los importes y los coloca en los Div marcados
 * @param params
 * @param t
 */
var reescribeImportes = function (params, t) {
    "use strict";
    var totalDatos = 0, dato = 0, x = 0, datos = t.responseText.split('#');
    totalDatos = datos.length;
    if (params.length > 0) {
        for (x in params) {
            if (dato < totalDatos) {
                $(params[x]).innerHTML = datos[dato];
                dato += 1;
            }
        }
    }
...
var borra = function (servicio) {
    ...
    if (confirm("Borrar servicio?")) {
        ...
        procesaAjax(pars, 'tabla', 'tabla', verServiciosContratados, false, false);
    }
};
/**
...
var modificar = function (servicio) {
    ...
    procesaAjax(pars, 'modificar', 'modificar', false, false, false);
};
/**
...
var recalcula = function () {
    "use strict";
    var pars, params, moneyUrl = '../inc/moneyHandler.php';
    pars = 'precio=' + $F('precio') + '&cantidad=' + $F('cantidad') +
        '&iva=' + $F('iva');
    params = {0: 'importe', 1: 'total'};
    procesaAjax(pars, false, false, reescribeImportes, params, moneyUrl);
};
/**
...
var frmAgregarServicio = function (cliente) {
    ...
    procesaAjax(pars, 'form_agregar', 'form_agregar', false, false, false);
};
/**
...
var valorServicio = function () {
    "use strict";
    var pars, servicio = $('servicios').value;
    pars = 'opcion=8&servicio=' + $('servicios').value;
    Ajax.Request(urlDatos,
        {
            method: 'post',
            parameters: pars,
...
var observaciones = function (servicio) {
    ...
    procesaAjax(pars, 'observa', 'observa', false, false, false);
};
/**
...
var clienteRango = function (opcion) {
    var url = 'nufact.php';
    var cliente = $F('idCliente');
    var pars = 'opcion=' + opcion;
    if (cliente != "" && cliente != 0) {
        procesaAjax(pars, 'parametros_facturacion', 'parametros_facturacion', false, false, url);
        camposFecha(opcion);
    } else {
        alert('Debe seleccionar un cliente');
    }
};
function camposFecha(opcion) {
    if (opcion == 1) {
        Calendar.setup({
