...
if (isset($_POST[cliente]))
{
	...
	$muestra .= "<div id='etq_cliente'>Cliente :<b>".utf8_encode($previo[1])."</b></div><input type='hidden' id='cliente' value='".$previo[0]."'/><input type='hidden' id='opcion' value='".$opcion."' /><br/>";
	$muestra .= "<div id='etq_resto'><span class='etiqueta'>Bultos:</span>&nbsp;<input type='text' id='bultos' size='3' value='".$s_bulto."'/>&nbsp;&nbsp;";
	$muestra .= "<span class='etiqueta'>Fecha Inicio:</span>(dd/mm/aaaa)&nbsp;<input type='text' id='finicio' value='".$s_finicio."'/><button type='reset' id='f_trigger_a'>...</button>&nbsp;&nbsp;";
	$muestra .= "<span class='etiqueta'>Fecha Fin:</span>(dd/mm/aaaa)&nbsp;<input type='text' id='ffinal' value='".$s_ffinal."'/><button type='reset' id='f_trigger_b'>...</button>&nbsp;&nbsp;";
	$muestra .= "<input type='submit' class='boton' id='btn_almacen' onclick='a_almacen()' value='".$boton."' /></div>";
	echo $muestra;
}
if(isset($_GET[tabla]))
{
	//$sql = "Select * from z_almacen where cliente like $_GET[tabla]";
        $sql = "Select * from z_almacen where cliente like $_GET[tabla] and (Year( now( ) ) - Year( inicio )) <=1";
        //echo $sql;
	$consulta = @mysql_db_query($dbname,$sql,$con);
	$muestra .= "<center><table  class='tabla' id='datos' cellspacing='2' cellpadding='2' style='width:500px'>
	<tr>
